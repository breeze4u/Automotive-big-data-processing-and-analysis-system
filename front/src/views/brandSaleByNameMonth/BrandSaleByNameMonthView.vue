<template>
  <div>
    <div class="block" style="margin-top: 10px">
      ...
      <el-date-picker style="width: 150px; margin-left: 5px" value-format="yyyy-MM" v-model="params.infor2"
...
export default {
  ...
  methods: {
    ...
    initEchartsBar(){
      request.get("/sales/barBrandSaleByNameMonth",{
        params: this.params
      }).then(res=>{
        if(res.code === '0'){
          let map = res.data
    ...
    initBar(xAxis,yAxis){
      ...
      option = {
        title: {
          text: this.params.infor1+'汽车品牌销售数据柱状图',
          subtext: this.params.infor2,
          left: 'center'
        },
        xAxis: {
          type: 'category',
    ...
    initLine(xAxis,yAxis){
      ...
      option = {
        title: {
          text: this.params.infor1+'汽车品牌销售数据折线图',
          subtext: this.params.infor2,
          left: 'center'
        },
        xAxis: {
    ...
    initEchartsBin(){
      request.get("/sales/binBrandSaleByNameMonth",{
        params: this.params
      }).then(res=>{
        if(res.code === '0'){
    ...
    initBin(data){
      ...
      option = {
        title: {
          text: this.params.infor1+'汽车品牌销售数据饼图',
          subtext: this.params.infor2,
          left: 'center'
        },
        tooltip: {
